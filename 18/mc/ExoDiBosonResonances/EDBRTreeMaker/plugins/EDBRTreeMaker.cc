    ...
    //  noiseFilterToken_(consumes<edm::TriggerResults>(iConfig.getParameter<edm::InputTag>("noiseFilter")))
{
    ...
    if (RunOnMC_){
        ...
        outTree_->Branch("ptgengl"           ,ptgengl         ,"ptgengl[5]/D"          );
        outTree_->Branch("etagengl"           ,etagengl         ,"etagengl[5]/D"          );
        outTree_->Branch("phigengl"           ,phigengl       ,"phigengl[5]/D"          );
        outTree_->Branch("egengl"           ,egengl         ,"egengl[5]/D"          );
...
EDBRTreeMaker::tightJetID( const pat::Jet& j ) {
    ...
    double CEMF = j.chargedEmEnergyFraction();
    double eta = j.eta();
    return ((  (NHF<0.90 && NEMF<0.90 && NumConst>1) && MUF <0.80 && CHF>0 && CHM>0 && CEMF<0.80 && abs(eta)<=2.6 ) || (  (NHF<0.90 && NEMF<0.99 ) && MUF <0.80 && CHM>0 && CEMF<0.80 && abs(eta)>2.6 && abs(eta)<=2.7 ) ||(  (NEMF>0.02 && NEMF<0.99 ) && NumNeutralParticle>2 && abs(eta)>2.7 && abs(eta)<=3.0 ) ||(  (NHF>0.2 && NEMF<0.9 ) && NumNeutralParticle>10 && abs(eta)>3.0 && abs(eta)<=5.0 ));
}
...
EDBRTreeMaker::tightJetIDpuppi( const pat::Jet& j ) {
    ...
    int CHM = j.chargedMultiplicity();
    double eta = j.eta();
    return ((  (NHF<0.90 && NEMF<0.90 && NumConst>1) && MUF <0.80 && CHF>0 && CHM>0 && CEMF<0.80 && abs(eta)<=2.6 ) || (  (NHF<0.90 && NEMF<0.99 ) && MUF <0.80  && CEMF<0.80 && abs(eta)>2.6 && abs(eta)<=2.7 ) ||(   NEMF<0.99  && abs(eta)>2.7 && abs(eta)<=3.0 ) ||(  (NHF>0.02 && NEMF<0.9 ) && NumNeutralParticle>2&& NumNeutralParticle<15 && abs(eta)>3.0 && abs(eta)<=5.0 ));
}
    ...
    //w and top info
        ...
        for(size_t ik=0; ik<genParticles->size();ik++)
        {
            if(abs((*genParticles)[ik].pdgId())==23)
            {
                ...
                const reco::Candidate* pztmp=findLastW(pztmp1,23);
...
void EDBRTreeMaker::setDummyValues() {
    ...
    for(int i=0;i<5;i++){
        ptgenwl[i]=-99;etagenwl[i]=-99;phigenwl[i]=-99;massgenwl[i]=-99;taggenwl[i]=-99;
